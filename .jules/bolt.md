...
## 2026-04-25 - Avoiding Regex Re-compilation in DOM Loops\n**Learning:** Inline regex literals (e.g., `/^message$/i`) inside DOM traversal loops or per-element processing blocks inside `page.evaluate` result in unnecessary regex recompilation and object allocation.\n**Action:** Hoist regex literals to the outermost scope of the `page.evaluate` block when they are static and applied to many elements.

...
async function prepareApplicationForm(page, job, config) {
    ...
    if (job.ats === 'greenhouse' || job.ats === 'lever' || job.ats === 'smartrecruiters') {
        await clickApplyEntryPoint(page).catch(e => console.warn(`      ⚠️ Note: Could not click apply entry point: ${e.message}`));
    }
...
async function main() {
    ...
    try {
        ...
        for (const job of actionableJobs) {
            ...
            if (tracker.enabled) {
                await tracker.updateApplicationResult(job, result, runDateIso).catch(e => console.error(`   ⚠️ Failed to update application result in tracker: ${e.message}`));
            }

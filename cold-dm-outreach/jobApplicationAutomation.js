...
async function discoverJobsForCompany(page, companyConfig, config) {
    ...
    return page.evaluate((companyName, hintFragments) => {
        // ⚡ Bolt: Hoist static patterns and configurations outside per-node helper functions
        // to prevent unnecessary memory allocations and regex recompilation on every DOM node inspected.
        ...
        function detectEmploymentType(text) {
            ...
            return EMPLOYMENT_PATTERNS.find((pattern) => normalized.includes(pattern)) || '';
        }
        ...
        function looksLikeRoleTitle(title, contextText) {
            ...
            if (ROLE_PATTERNS.some((pattern) => pattern.test(normalizedTitle))) {
                return true;
            }

            return normalizedTitle.split(' ').length >= 2 &&
                ROLE_PATTERNS.some((pattern) => pattern.test(normalizedContext));
        }
        ...
        function isGenericCareerNavigation(title, url) {
            ...
            return GENERIC_TITLE_PATTERNS.some((pattern) => pattern.test(normalizedTitle)) ||
                GENERIC_URL_FRAGMENTS.some((fragment) => normalizedUrl.includes(fragment));
        }
